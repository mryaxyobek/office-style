...
const HomePageNewProducts = () => {
    ...
    // all products
    const allProducts = [...cabinetProductsForManagers, ...furnitureForStaff, ...officeSofas, ...managersChair, ...officeChair, ...receptionDesks, ...courtFurnitures, ...furnitureForCallCenter].filter((product) => {
        let total = 0;
        product.reviews.map((review) => {
            total += review.rating;
        });
        return (total / product.reviews.length) > 4.5;
    });
