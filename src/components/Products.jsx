    ...
    return (
        <div className="container">
            ...
            <ul className="grid grid-cols-4 gap-8 max-1400:grid-cols-3 max-1050:grid-cols-2 max-730:grid-cols-1">
                {cabinetProductsForManagers.map((e, index) => {
                    return (
                        <li key={e.id} className="w-full product hover:active-hover">
                            ...
                            {/* title  */}
                            <h3 className="mb-4 text-regular-20">{e.productTitle}</h3>
                            ...
                            <ul className="text-regular-14 text-primary-gray-70 space-y-2.5 mb-4">
                                <li className="flex items-end">
                                    ...
                                    <span>{e.details.country}</span>
                                </li>
                                <li className="flex items-end">
                                    <span>Ustki material</span>
                                    <div className="grow border-t-2 mx-1 mb-0.5 border-primary-gray-70 border-dotted"></div>
                                    <span>{e.details.material}</span>
                                </li>
                                <li className="flex items-end">
                                    <span>Ustki qalinlik</span>
                                    <div className="grow border-t-2 mx-1 mb-0.5 border-primary-gray-70 border-dotted"></div>
                                    <span>{e.details.thickness}</span>
                                </li>
                            </ul>
                            ...
                            <div className="flex-c-b max-440:flex-col max-440:items-start max-440:gap-6 mb-5">
                                <div className="flex-end max-340:flex-col max-340:items-start gap-1">
                                    ...
                                    <p className="text-regular-20 mr-3">{e.price.currentPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}so'm</p>

                                    {/* old price aksiya price  */}
                                    {
                                        e.price.oldPrice && <del className='text-regular-18 text-primary-gray-50'>{e.price.currentPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}so'm</del>
                                    }
                                </div>
                            ...
                            <div className="flex flex-co w-full gap-2">
                                <Link to={`/catalog/${categoryName}/${e.productTitleNoSpacing}`} className="flex-center justify-center red-btn py-2.5 px-5 w-full bg-primary-red-60">Buyurtma berish</Link>
                                <button className="red-btn py-2.5 px-8 bg-transparent border-2 border-primary-red-60 ">
