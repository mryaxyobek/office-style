...
const ProductDesignHero = () => {
    ...
    const decrementCount = (index) => {
        ...
        if (updatedProductParts[index].count > 0) {
    ...
    const incrementCount = (index) => {
        const updatedProductParts = [...productParts];
        if (updatedProductParts[index].count < 99) {
            updatedProductParts[index].count++;
            setProductParts(updatedProductParts);
        }
    };
        ...
        <>
            <section className='pt-20 max-1150:pb-16 max-900:pt-8 max-800:pb-14 max-580:pb-12 max-540:pt-0 max-440:pb-8 pb-60r'>
                <div className="container">
                                                    ...
                                                    productParts.map((part, index) => {
                                                        return (
                                                            productParts.length > 1 ?
                                                                ...
                                                                :
                                                                <li key={part.id} className='flex-center gap-8'>
                                                                    <div className="flex">
                                                                        <button onClick={() => decrementCount(index)} className="red-btn !bg-primary-red-50 disabled:!bg-primary-gray-10 disabled:text-secondary-blue-70 rounded w-11 h-11">-</button>
                                                                        <span className='flex-center justify-center text-black w-10 mx-2 text-regular-28'>{part.count}</span>
                                                                        <button onClick={() => incrementCount(index)} className="red-btn !bg-primary-red-50 rounded w-11 h-11 disabled:!bg-primary-gray-10 disabled:text-secondary-blue-70">+</button>
                                                                    </div>

                                                                    <div className="space-y-1">
                                                                        {
                                                                            part.oldprice &&
                                                                            <del className="text-regular-16 text-primary-gray-50">{part.oldprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}so'm</del>
                                                                        }
                                                                        <p className="text-regular-28">{part.currentPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}so'm</p>
                                                                    </div>
                                                                </li>
                                                        )
                                                    })
                    ...
                    {/* modal parent  */}
                    <div className={`${openModal ? 'flex-center' : 'hidden'} flex-center' flex-center fixed justify-center top-0 right-0 w-full h-screen z-50`}>
                        {/* overlay */}
                        ...
                        <div className="flex relative w-full max-w-1088px h-full max-h-640px max-730:justify-center max-580:max-h-full">
                            ...
                            >
                                <div className="my-auto">
                                    ...
                                    <p className="text-regular-12 text-center">Saytga ma'lumot yuborish orqali siz <Link to='/policy' className='text-primary-red-50'>Shaxsiy ma'lumotlarni himoya qilish siyosati shartlarini qabul qilasiz</Link></p>
                                </div>
                            </form>
