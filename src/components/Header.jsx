                ...
                <div className="z-3 absolute w-full -bottom-20 left-0 py-18px transition-colors-2 max-900:hidden bg-transparent">
                    <div className="flex-c-b container">
                        ...
                        <div className="flex-center space-x-3.5">
                            {/* search */}
                            <div className={`relative transition-transform-2`}>
                                <form onSubmit={preventdefault} className="flex-center justify-center relative">
                                    {/* search input  */}
                                    <div className={`absolute z-0 right-full transition-opacity-2 ${openSearchInput ? 'block' : 'hidden'}`}>
                                        <input placeholder={`${openSearchInput ? 'Nimani qidiramiz?' : ''}`} type="text" className={`w-394px h-41px leading-17px text-restangle-14 py-11px search-input`} />

                                        {/* search results  */}
                                    </div>

                                    {/* search btn */}
                                    <button type='button' onClick={openSearchInputFunc} className={`${openSearchInput ? 'border-l-0 search-btn bg-white' : ''} ${changeText ? openSearchInput ? 'border-secondary-gray-50' : 'border-white' : openSearchInput ? 'border-secondary-gray-50' : 'border-black'} transition-all duration-300 relative border z-1 p-11px rounded-lg`}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                            ...
                            {/* basket link  */}
                            <Link to='/basket' className={`inline-block  transition-all duration-300  ${openSearchInput ? '!rounded-100%' : 'rounded-lg'} ${changeText ? 'border-white' : 'border-black'} border relative p-11px`}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
