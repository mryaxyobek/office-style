...
import { openCallModal } from '../store/slices/modalSlice';
...
const Header = () => {
    ...
    const filteredProducts = (event) => {
        ...
        if (value != '' && value != ' ' && value != '  ') {
            const filterdProducts = allProducts.filter((product) => product.productTitle.toLowerCase().includes(value.toLowerCase()) || product.type.toLowerCase().includes(value.toLowerCase()));
        ...
        <header className={`py-4 bg-primary-gray-04 relative`}>
            <div className="flex-c-b container">
                ...
                <div className='flex-center'>
                    ...
                    <button
                        onClick={() => {
                            openCallModalFunction();
                            setOpenCatalog(false);
                        }}
                        className="flex items-center justify-center bg-primary-gray-80 rounded-lg max-1200:!text-xs py-3.5 px-6 text-white text-regular-14 max-1050:bg-transparent max-1050:w-9 max-1050:h-9 max-1050:border max-1050:p-0 max-1050:border-primary-gray-20 max-900:mr-4 max-730:mr-3 max-670:mr-2">
                    ...
                    <div className="hidden space-x-4 max-900:flex max-670:space-x-2 max-730:space-x-3">
                        ...
                        <Link to='/basket' className={`flex-center justify-center w-9 h-9 rounded-lg border-primary-gray-20 border relative`}>
                            ...
                            {/* num  */}
                            <span className='flex-center justify-center leading-3 absolute -top-3px -right-3px w-18px h-18px rounded-full bg-primary-red-60 text-white text-xs'>{card.length}</span>
                        </Link>
                        ...
                        }} className={`${openCatalog || openSearchInput ? 'fixed top-4 right-5' : ''} flex-center red-btn justify-center w-9 h-9 z-40`}>
                            {/* cross  */}
                            <span className={`${openCatalog || openSearchInput ? '-rotate-45' : 'rotate-0'} flex-center transition-transform-2 justify-between w-4 h-4 relative`}>
                                <span className={`${openCatalog || openSearchInput ? 'translate-y-0 w-4 rotate-90' : '-translate-y-5px w-3.5'} inline-block absolute transition-sizing-and-transfrom bg-white h-0.5`}></span>
                                <span className={`${openCatalog || openSearchInput ? 'opacity-0' : 'opacity-100'} inline-block absolute transition-opacity bg-white h-0.5 w-4`}></span>
                                <span className={`${openCatalog || openSearchInput ? 'translate-y-0 w-4' : 'translate-y-5px w-2.5'} inline-block absolute transition-sizing-and-transfrom bg-white h-0.5`}></span>
                            </span>
                        </button>
                    </div>
                </div>
                ...
                {/* bottom content  */}
                <div className="z-3 absolute w-full -bottom-20 left-0 py-18px transition-colors-2 max-900:hidden bg-transparent">
                    <div className="flex-c-b container">
                        <div className="flex-center">

                            {/* catalog btn  */}
                            <button onClick={openCatalogFunc} className="flex-center red-btn py-3 px-4 mr-10 max-440:z-50">
                                {/* cross  */}
                                ...
                                {/* text  */}
                                <span className='text-regular-16'>Sahifalar</span>
                            </button>
                            ...
                            <nav className={`${openSearchInput ? 'hidden' : 'block'} max-1350:hidden`}>
                                <ul className={`${openCatalog ? 'black' : changeText ? 'text-white' : ''} flex-center space-x-10 text-regular-16`}>
                                    ...
                                    <li>
                                        <Link to='/catalog/ofis-divani'>Ofis divani</Link>
                                    </li>
                                    <li>
                                        <Link to='/catalog/qabul-qilish-stoli'>Qabul qilish stollari</Link>
                                    </li>
                                </ul>
                            </nav>
                        </div>

                        {/* search btn and basket link */}
                        <div className="flex-center space-x-3.5">
                            ...
                            {/* overlay */}
                            <div onClick={() => setOpenSearchInput(false)} className={`${openSearchInput ? 'block' : 'hidden'} fixed w-full h-screen top-0 right-0 bg-black z-1 bg-opacity-75`}></div>

                            {/* basket link  */}
                            <Link to='/basket' className={`flex-center justify-center w-10 h-10 transition-all duration-300  ${openSearchInput ? '!rounded-100%' : 'rounded-lg'} ${changeText ? 'border-white' : 'border-black'} border relative`}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                                ...
                                {/* num  */}
                                <span className={`flex-center justify-center leading-3 absolute -top-3px -right-3px  rounded-full bg-primary-red-60 text-white text-xs ${card.length > 99 ? 'w-6 h-6 -top-1.5 -right-1.5' : 'w-18px h-18px'}`}>{card.length > 99 ? <span>99+</span> : card.length}</span>
                            </Link>
                        </div>
                    </div>
                </div>
            ...
            {/* search place 900px dan keyin */}
            <div className={`${openSearchInput ? 'translate-x-0' : '-translate-x-full'} transition-transform-2  hidden max-900:block fixed w-full h-screen top-0 right-0 z-30 bg-white p-6 max-470:px-5`}>


                {/* header */}
                <div className="fixed w-full px-6 pt-6 top-0 right-0">
                    ...
                    <div className="flex relative w-full h-11 items-center">
                        <input type="text" onKeyDown={filteredProducts} placeholder='Nimani qidiramiz?' className='pr-12' />

                        <svg className='absolute right-4' xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                            <path fillRule="evenodd" clipRule="evenodd" d="M15.5355 16.7363L11.6926 12.8863C8.69326 15.2551 4.36773 14.8755 1.82676 12.0205C-0.71421 9.16545 -0.590064 4.8244 2.10978 2.11923C4.80975 -0.585937 9.1498 -0.71775 12.0089 1.81858C14.868 4.3549 15.2552 8.68042 12.8922 11.6844L16.7351 15.5344C16.9591 15.7468 17.0502 16.0639 16.973 16.3628C16.8959 16.6616 16.6629 16.8951 16.3642 16.9727C16.0655 17.0503 15.7483 16.9598 15.5357 16.7363L15.5355 16.7363ZM1.71634 7.21456C1.7109 9.43369 3.03684 11.4397 5.08058 12.3041C7.12419 13.1685 9.48685 12.7226 11.0752 11.1729C11.0915 11.1537 11.1085 11.1353 11.1263 11.1176C11.1443 11.0995 11.1624 11.0826 11.1815 11.0666C12.9256 9.28543 13.2599 6.55614 11.9971 4.40655C10.7344 2.25695 8.18785 1.2205 5.78321 1.87734C3.37857 2.53419 1.71232 4.72137 1.71737 7.21453L1.71634 7.21456Z" fill='black' />
                        </svg>
                    </div>
                </div>
                ...
                {
                    products.length > 0 ?
                        <div className="red-scroll overflow-y-auto h-full mt-36">
                            <ul className="flex flex-col gap-3 w-full pb-40">
                                {
                                    products.map((product, index) => {
                                        return (
